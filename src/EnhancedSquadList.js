...
function EnhancedSquadList({ fixtures = [], isAuthenticated, onRequestLogin, user }) {
  ...
  // Fetch all squad data from squad2526f â€” superset of all squad fields
  // Financial fields only displayed when isAuthenticated is true
  useEffect(() => {
    ...
    fixtures.forEach(fixture => {
      ...
      for (let i = 1; i <= 11; i++) {
        ...
        if (starter && starter.includes(playerRef)) {
          minutesThisGame = MATCH_DURATION; playedThisGame = true; break;
        }
      }
      for (let i = 1; i <= 5; i++) {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        if (subOn && subOn.includes(playerRef) && subTime) {
          const minute = parseMinute(subTime);
          if (minute !== null) { minutesThisGame = MATCH_DURATION - minute; playedThisGame = true; }
          break;
        }
      }
      for (let i = 1; i <= 2; i++) {
  ...
  if (selectedPlayer) {
    return <PlayerDetail player={selectedPlayer} onBack={() => setSelectedPlayer(null)} user={user} />;
  }
      ...
      {filteredPlayers.map(player => {
        ...
        return (
          <div key={player.id}
            ...
            style={{
              border: '1px solid #ddd', cursor: user ? 'pointer' : 'default',
            ...
            {isAuthenticated && (
              <div style={{ backgroundColor: '#fff3cd', border: '1px solid #ffc107', borderRadius: '5px', padding: '10px', marginTop: '12px' }}>
